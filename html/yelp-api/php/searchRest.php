...
if (empty($_POST['sendValue'])) {
    ...
    echo json_encode(array("returnValueName"=>"This is name from PHP : ".name($response, 0), "returnValueId"=>"This is id from PHP : ".id($response, 0), "returnValueRating"=>rating($response,0), "returnValueRatingImg"=>ratingimg($response,0), "returnValueSnippet"=>snippet($response,0)));
}
